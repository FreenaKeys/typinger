...
### Phase 6: 実験モード（開発中）
**目的**: 高校生5名によるキーボード配列比較実験のサポート

#### Phase 6 開発計画（13コミット）

1. **実験用シナリオファイル追加** ✅
   - 6セッション × 6テスト = 36個のJSONファイル
   - JMdict辞書から文字頻度を考慮して生成

2. **実験セッション管理基盤** ✅
   - ExperimentSession構造体定義
   - 被験者ID入力・検証機能（SK/MT/IK/KS/KM）
   - セッション番号入力・検証機能（1-14）
   - 配列交互実施ロジック
...
6. **シナリオ選択システム**
   - セッション番号に基づく自動シナリオ選択機能
   - scenario/experiment/からファイル読み込み
...
**実験設計概要:**
- 被験者: 5名（SK, MT, IK, KS, KM）
- セッション: 週1回 × 6週間（計14セッション、予備含む）
- テスト構成: 1セッション6回（新旧配列を3回ずつ交互）
- テスト時間: 2分/回
- CSV出力: 被験者ID・セッション番号・配列タイプを含む詳細データ
